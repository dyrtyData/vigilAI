...
```
vigilai eval hf/Qwen/Qwen3-8B -M device=cuda:0
```
...
```
vigilai eval hf/local -M model_path=./my-model
```
...
```
vigilai eval vllm/local -M model_path=./my-model
```
...
```
vigilai eval vllm/Qwen/Qwen3-8B -M tool-call-parser=hermes
```
...
vigilai eval openai-api/deepseek/deepseek-reasoner
```
...
```
vigilai eval openai-api/<provider>/<model> -M responses_api=true
```
