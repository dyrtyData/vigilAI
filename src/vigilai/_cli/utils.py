...
def get_vigilai_tasks(
    filter: Callable[[TaskInfo], bool] | None = None,
) -> list[TaskInfo]:
    """
    Get all available tasks implemented in vigilAI.
...
def get_task_infos_from_task_names(
    ...
    tasks_to_run = get_vigilai_tasks(filter=filter)
    ...
    if "swe_bench_verified" in {task.name for task in task_infos} and (
        ...
        if "tool_call_parser" not in model_args:
            raise typer.BadParameter(
                ...
                "If running your own server, specify the tool call parser in your server launch command and provide a dummy value to vigilai. "
                "If not running your own server, specify the tool call parser as a model argument to vigilai: "
...
def patch_display_results() -> None:
    """
    Replace "inspect eval-retry" with "vigilai eval-retry" in display results.
    ...
    try:
        ...
        def custom_task_interrupted(profile, samples_completed):  # type: ignore
            ...
            if isinstance(result, str):
                return result.replace("inspect eval-retry", "vigilai eval-retry")
                    ...
                    if isinstance(segment, tuple) and len(segment) >= 1:
                        ...
                        if isinstance(text, str) and "inspect eval-retry" in text:
                            new_text = text.replace(
                                "inspect eval-retry", "vigilai eval-retry"
                            )
